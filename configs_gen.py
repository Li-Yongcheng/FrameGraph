...
def gen_grid(args, config, config_budget={}):
    task_name = f'{get_fname(args.config)}_grid_{get_fname(args.grid)}'  # 配置文件夹，tudataset_graph_grid_tudataset_graph
    fname_start = 'True'  # 固定参数值作为文件名开头 [True, False]
    # fname_start = get_fname(args.config)  # tudataset_graph
    out_dir = f'{args.out_dir}/{task_name}'
    makedirs_rm_exist(out_dir)
    config['out_dir'] = os.path.join(config['out_dir'], task_name)  # 结果文件夹
    ...
    for i, out in enumerate(outs):
        ...
        random.shuffle(vars_value)
        vars_value = vars_value[:60]  # 从设计空间取100个配置进行实验
        if i == 0:
            print(f'Variable label: {vars_label}')
            print(f'Variable alias: {vars_alias}')

        for idx, vars in enumerate(vars_value):
            config_out = config.copy()
            fname_out = fname_start  # tudataset_graph，名字太长，算不了
            # fname_out = str(idx)
            for id, var in enumerate(vars):  # 开始新建配置文件
